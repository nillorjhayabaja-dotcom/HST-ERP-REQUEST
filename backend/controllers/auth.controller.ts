...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign({ userId: profile.id, email: profile.email }, jwtSecret, {
      expiresIn: process.env.JWT_EXPIRES_IN || "15m",
    });

    const refreshSecret = process.env.JWT_REFRESH_SECRET;
    const refreshToken = jwt.sign({ userId: profile.id }, refreshSecret || jwtSecret, {
...
export const refreshToken = async (req: Request, res: Response) => {
  try {
    ...
    const newToken = jwt.sign({ userId: profile.id, email: profile.email }, jwtSecret, {
      expiresIn: process.env.JWT_EXPIRES_IN || "15m",
    });

    const newRefreshToken = jwt.sign({ userId: profile.id }, refreshSecret, {
      expiresIn: process.env.JWT_REFRESH_EXPIRES_IN || "7d",
    });
