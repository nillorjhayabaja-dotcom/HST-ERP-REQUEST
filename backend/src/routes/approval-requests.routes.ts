  ...
  try {
    ...
    const [requests, total] = await Promise.all([
      prisma.approvalRequest.findMany({
        ...
        include: { workflow: true, requester: { select: { id: true, full_name: true } }, actions: true },
...
router.post('/', authenticate, requirePermission('approval_requests', 'create'), async (req, res) => {
  try {
    const { module, entity_type, entity_id, workflow_id, notes } = req.body;
    const request = await prisma.approvalRequest.create({
      data: { module, entity_type, entity_id, workflow_id, notes, requested_by: (req as any).user?.id, status: 'pending', current_step: 0 },
    });
    await logAudit({ user_id: (req as any).user?.id, module: 'approval_requests', action: 'create', entity_type, entity_id, new_value: request });
...
router.post('/:id/approve', authenticate, requirePermission('approval_requests', 'update'), async (req, res) => {
  try {
...
router.post('/:id/reject', authenticate, requirePermission('approval_requests', 'update'), async (req, res) => {
  try {
    const { comment } = req.body;
