...
router.post('/verify-qr', requirePermission('gate_passes', 'read'), async (req, res) => {
  try {
    ...
    if (!gatePass) return res.status(404).json({ error: 'Invalid Pass', message: 'Gate pass not found' });
    if (gatePass.deleted_at) return res.status(404).json({ error: 'Invalid Pass', message: 'Gate pass has been cancelled' });
    if (gatePass.status === 'rejected') return res.status(400).json({ error: 'Rejected', message: 'Gate pass has been rejected' });
    if (gatePass.status === 'cancelled') return res.status(400).json({ error: 'Cancelled', message: 'Gate pass has been cancelled' });
    if (gatePass.status === 'expired') return res.status(400).json({ error: 'Expired', message: 'Gate pass has expired' });
    if (gatePass.status === 'completed') return res.status(400).json({ error: 'Already Used', message: 'Gate pass has already been used' });
    if (gatePass.status !== 'approved' && gatePass.status !== 'released') {
      return res.status(400).json({ error: 'Invalid Status', message: `Gate pass status is: ${gatePass.status}` });
    }
